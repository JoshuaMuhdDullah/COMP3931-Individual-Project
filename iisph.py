# Implementation based on https://github.com/InteractiveComputerGraphics/physics-simulation/blob/main/examples/iisph.html
...
class IISPHSimulation:
                ...
                for slot in range(count):
                    ...
                    if j != i:
                        ...
                        if dist2 <= radius2 + 1.0e-6:
                            ...
                            if j < self.num_fluid:
                                Drho_Dt += self.mass_value * (
                                    (vxi - self.vx[j]) * grad_x
                                    + (vyi - self.vy[j]) * grad_y
                                )
                            else:
                                Drho_Dt += self.psi[j] * (
                                    vxi * grad_x + vyi * grad_y
                                )

            self.source_term[i] = (1.0 / dt) * (
                self.density0 - (self.density[i] + dt * Drho_Dt)
            )
                ...
                for slot in range(count):
                    ...
                    if j != i:
                        ...
                        if dist2 <= radius2 + 1.0e-6:
                            ...
                            if j < self.num_fluid:
                                val += self.mass_value * (
                                    (self.pax[i] - self.pax[j]) * grad_x
                                    + (self.pay[i] - self.pay[j]) * grad_y
                                )
                            else:
                                val += self.psi[j] * (
                                    self.pax[i] * grad_x + self.pay[i] * grad_y
                                )

            self.aij_pj[i] = val * dt

    @ti.kernel
    def update_pressure(self) -> ti.f32:
        density_err = 0.0
    ...
    @ti.kernel
    def integrate_pressure_velocity_and_position(self):
        for i in range(self.num_fluid):
    ...
    def pressure_solve(self):
        ...
        while (
            (self.avg_density_error > threshold and self.iterations_used < self.max_iterations)
            or self.iterations_used < 2
        ):
            self.compute_pressure_accelerations()
            self.compute_aij_pj()

            self.avg_density_error = float(self.update_pressure())
    ...
    def simulation_step(self):
        ...
        self.integrate_pressure_velocity_and_position()
...
def main():
    sim = IISPHSimulation(WIDTH_PARTICLES, HEIGHT_PARTICLES)
    ...
    # log_file = open("density_error_iisph_dt2ms.csv", "w")
    ...
    # log_file = open("kinetic_energy_iisph.csv", "w")
    # log_file.write("step,time,kinetic_energy\n")

    # kinetic_energy = 0.0

    # # Logging pressure stats to CSV
    # pressure_log_file = open("pressure_stats_iisph.csv", "w")
    ...
    while gui.running:
        ...
        gui.text(
            f"IISPH iterations = {sim.iterations_used} | density error = {sim.avg_density_error:.5f}",
            pos=(0.02, 0.88),
            color=0x000000,
        )
        gui.text(
